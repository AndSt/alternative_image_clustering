...
from alternative_image_clustering.experiments.single_experiments import (
    ...
    run_enrc_clusterings,
    run_orth1_clusterings,
    run_orth2_clusterings
)


def main(base_dir: str = "data", dataset: str = None):
    cache_dir = "benchmark_cache_old"
    datasets = ["cards", "fruit360", "gtsrb", "nrobjects"]
    if dataset in datasets:
        datasets = [dataset]

    num_exps = len(datasets) * (5 + 2 * 4)
    ...
    for dataset in datasets:
        ...
        enrc_file = os.path.join(save_dir, "enrc.pbz2")
        if not os.path.exists(enrc_file):
            results = run_enrc_clusterings(
                dataset, embedding_type="image", base_dir=base_dir
            )
            joblib.dump(results, enrc_file)
        pbar.update(1)

        orth_1_file = os.path.join(save_dir, "orth_1.pbz2")
        if not os.path.exists(orth_1_file):
            results = run_orth1_clusterings(
                dataset, embedding_type="image", base_dir=base_dir
            )
            joblib.dump(results, orth_1_file)
        pbar.update(1)

        orth_2_file = os.path.join(save_dir, "orth_2.pbz2")
        if not os.path.exists(orth_2_file):
            results = run_orth2_clusterings(
                dataset, embedding_type="image", base_dir=base_dir
            )
            joblib.dump(results, orth_2_file)
        pbar.update(1)
